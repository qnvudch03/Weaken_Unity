...
public class ManoeuvreUpDown : MonoBehaviour, IManoeuvreBehaviour
{
    ...
    {
        StartCoroutine(MoveUpDown(dron));
    }

    IEnumerator MoveUpDown(Drone_11 dron)
    {
        float time;
        bool isReverse = false;
        float speed = dron.speed;
        Vector3 startPosition = dron.transform.position;
        Vector3 endPosition = new Vector3(startPosition.x, startPosition.y + dron.maxMovingHeight, startPosition.z);
